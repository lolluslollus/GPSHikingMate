...
{
	public sealed partial class Main : Page, IFileOpenPickerContinuable, IFileActivatable, IFileSavePickerContinuable
	{
		...
		{
			if (MyPersistentData.IsShowingPivot && !e.Handled)
			{
				e.Handled = true;
		...
		private void UpdateOrientation()
		{
			...
			switch (_appView.Orientation)
			{
				case ApplicationViewOrientation.Landscape:
					BottomButtonBar.Visibility = Visibility.Collapsed;
					LeftButtonBar.Visibility = Visibility.Visible;
					break;
				case ApplicationViewOrientation.Portrait:
					LeftButtonBar.Visibility = Visibility.Collapsed;
					BottomButtonBar.Visibility = Visibility.Visible;
					break;
				default:
					break;
			}
		...
		{
			// _myVM.PickLoadRoute0();
			_myVM?.PickLoadSeries(PersistentData.Tables.Route0);
		}

		private void OnSaveTrackingHistory_Click(object sender, RoutedEventArgs e)
		{
			_myVM.PickSaveSeries(PersistentData.Tables.History, "_" + ConstantData.APPNAME_ALL_IN_ONE + "_Route");
		}

		private void OnSaveRoute0_Click(object sender, RoutedEventArgs e)
		{
			_myVM.PickSaveSeries(PersistentData.Tables.Route0, "_" + ConstantData.APPNAME_ALL_IN_ONE + "_Route");
		}
		...
		{
			// _myVM.PickLoadLandmarks();
			_myVM?.PickLoadSeries(PersistentData.Tables.Landmarks);
		}
		...
		{
			_myVM.PickSaveSeries(PersistentData.Tables.Landmarks, "_" + ConstantData.APPNAME_ALL_IN_ONE + "_Landmarks");
		}
		...
		public Task ContinueFileOpenPickerAsync(FileOpenPickerContinuationEventArgs args)

		{
			return _myVM.ContinueFileOpenPickerAsync(args);

		}

		public Task ContinueFileSavePickerAsync(FileSavePickerContinuationEventArgs args)
		{
			return _myVM.ContinueFileSavePickerAsync(args);
		}
